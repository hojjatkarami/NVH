...
x0(19:27) = -.1;
x0(end)=50;
options = optimoptions('fmincon','Display','iter','TolX',1e-200,'MaxFunEvals',100000);
f=fmincon(@myfun2,x0,[],[],[],[],lb,ub,[],options);
...
c_l_1 = diag([f(1) f(4) f(7)])*0.01;
c_l_2 = diag([f(2) f(5) f(8)])*0.01;
c_l_3 = diag([f(3) f(6) f(9)])*0.01;
c_1 = A_1*c_l_1*A_1';
c_2 = A_2*c_l_2*A_2';
c_3 = A_3*c_l_3*A_3';
...
figure;plot(t,EE(:,4:6),'linewidth',1)
title('Angles in Princapl Moment of Inertia Coordinates','fontsize',18);
xlabel('Time (s)','fontsize',15);
ylabel('Angles in radian','fontsize',15);
legend('\theta_X','\theta_Y','\theta_Z');
